  ...
  return (
    <div className="product-viewer">
      ...
      <div className="websites">
        {Object.entries(props.product.websites)
          .filter(([k, v]) => v && v?.length > 0)
          .map(([k, v]) => (
            <a
              href={v}
              className="product-link"
              style={{ backgroundColor: websiteToColor?.[k] ?? "black" }}
            >
              {k}
            </a>
          ))}
      ...
      <p className="brand-and-type">
        מותג:
        <Link className="brand" to={`/brands/${props.product.brand}`}>
          {" "}
          {props.product.brand}
        </Link>
        , סוג מוצר:
        <Link
          className="type"
          to={`/product_type/${props.product.product_type}`}
        >
          {" "}
          {props.product.product_type
            .map((t) => productTypeToHebrew[t] ?? t)
            .join(", ")}
        </Link>
      </p>
